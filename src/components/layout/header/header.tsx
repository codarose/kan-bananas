...
const headerStyles =
  'box-border flex flex-1 dark:bg-darkGrey items-center px-5 gap-5 basis-full justify-between font-semibold w-screen h-[100px] bg-white border-b border-lightLines dark:border-darkLines';
    ...
    <div className={headerStyles}>
      {isDarkMode ? (
        <div className={logoWrapper}>
          <img className={logoStyle} src="/icons/logo-light.svg" />{' '}
        </div>
      ) : (
        <div className={logoWrapper}>
          <img className={logoStyle} src="/icons/logo-dark.svg" />
        </div>
      )}

      <div className={currentBoardLabel}>{selectedBoard}</div>

      <Button variant={ButtonVariant.secondary}>+ Add New Task</Button>
      <Button variant={ButtonVariant.empty}>
        <img src="/icons/icon-vertical-ellipsis.svg" />
      </Button>
    </div>
  );
}
